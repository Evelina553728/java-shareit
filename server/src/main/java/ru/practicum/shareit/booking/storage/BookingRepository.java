...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    List<Booking> findLastApproved(@Param("itemId") long itemId, @Param("status") BookingStatus status, @Param("now") LocalDateTime now);

    @Query("select b from Booking b where b.item.id = :itemId and b.status = :status and b.start > :now order by b.start asc")
    List<Booking> findNextApproved(@Param("itemId") long itemId, @Param("status") BookingStatus status, @Param("now") LocalDateTime now);
